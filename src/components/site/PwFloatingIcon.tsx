...
export function PwFloatingIcon() {
  ...
  const [showTip, setShowTip] = useState<"pw" | "ai" | null>("pw");

  useEffect(() => {
    setShowTip("pw");
    const t = setTimeout(() => setShowTip(null), 6500);
  ...
  return (
    <div className="fixed bottom-20 right-4 z-40 flex flex-col items-end gap-2 sm:bottom-24 sm:right-6 sm:gap-3">
      {/* PW Icon */}
      ...
      <div className="relative">
        ...
        <Link
          to="/ai"
          aria-label="Open NiteshAI Assistant"
          className="group relative inline-flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border-2 border-ink bg-foreground shadow-elevated transition-transform hover:scale-105 active:scale-95 sm:h-16 sm:w-16"
          onMouseEnter={() => setShowTip("ai")}
        >
          <Sparkles className="h-6 w-6 text-background sm:h-7 sm:w-7" />
          <span className="absolute inset-0 rounded-full ring-2 ring-primary/0 transition group-hover:ring-primary/40" />
          <span className="absolute -right-0.5 -top-0.5 inline-flex h-3.5 w-3.5 items-center justify-center rounded-full border-2 border-background bg-primary" />
        </Link>
      </div>
